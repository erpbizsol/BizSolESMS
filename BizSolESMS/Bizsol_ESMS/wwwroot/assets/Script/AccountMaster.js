            ...
            if (response) {
                if (response.AccountMaster && response.AccountMaster.length > 0) {
                    ...
                    if (Array.isArray(accountMaster.BrandName)) {
                        codes = accountMaster.BrandName;
                    } else if (accountMaster.BrandName) {
                        codes = accountMaster.BrandName.split(',')
                            .map(x => x.trim())
                            .filter(x => x.length > 0);
                    }
                    ...
                    setTimeout(function () {
                        $('.option').each(function () {
                            ...
                            if (codes.includes(brandName) || codes.includes(brandValue)) {
...
function addNewRowEdit(index, address) {
    ...
    newRow.innerHTML = `
        <td><input type="text" class="txtAddressCode box_border form-control form-control-sm" id="txtAddressCodeby_${rowCount}" autocomplete="off" required maxlength="20" /></td>
        ...
        <td><input type="text" class="txtEmail box_border form-control form-control-sm" id="txtEmailby_${rowCount}" autocomplete="off"maxlength="100" /></td>
...
function BrandList() {
    $.ajax({
        url: `${appBaseURL}/api/Master/GetBrandList`,
        type: 'GET',
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Auth-Key', authKeyData);
        },
