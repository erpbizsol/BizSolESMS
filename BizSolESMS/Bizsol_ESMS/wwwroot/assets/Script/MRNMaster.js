...
function getMRNFixRow() {
    if (!G_Fixparameter) return {};
    return Array.isArray(G_Fixparameter) ? (G_Fixparameter[0] || {}) : G_Fixparameter;
}
function getMRNFixFlag(name) {
    const row = getMRNFixRow();
    const camel = name.charAt(0).toLowerCase() + name.slice(1);
    const value = row[name] != null ? row[name] : row[camel];
    return (value == null ? '' : String(value)).trim().toUpperCase();
}
function isVehicleNoMandatoryOnMRN() {
    const flag = getMRNFixFlag('IsVehicleNoMandatoryOnMRN');
    if (flag) return flag === 'Y';
    return getMRNFixFlag('MendateVehicleNo') === 'Y';
}
function isBillNoMandatoryOnMRN() {
    return getMRNFixFlag('IsBillNoMandatoryOnMRN') === 'Y';
}

$(document).ready(function () {
    if (isVehicleNoMandatoryOnMRN()) {
    ...
    if (isBillNoMandatoryOnMRN()) {
        $("#spanImportBillNoRequired").show();
    } else {
        $("#spanImportBillNoRequired").hide();
    }
    ...
    $('#txtImportVehicleNo').on('keydown', function (e) {
        if (e.key === "Enter") {
            $("#txtImportBillNo").focus();
        }
    });
    $('#txtImportBillNo').on('keydown', function (e) {
        if (e.key === "Enter") {
...
function Save() {
    ...
    if (MRNDate == '') {
        ...
        $("#txtChallanDate").focus();
        return;
    } else if ((VehicleNo || '').trim() == '' && isVehicleNoMandatoryOnMRN()) {
...
function SaveImportFile() {
    ...
    const BillNo = ($("#txtImportBillNo").val() || "").trim();
    ...
    if (VendorName == '') {
        ...
        $("#txtImportVehicleNo").focus();
        return;
    } else if (BillNo == '' && isBillNoMandatoryOnMRN()) {
        toastr.error("Please enter Bill No/LR Number !");
        $("#txtImportBillNo").focus();
        return;
...
function GetImportFile() {
    ...
    $.ajax({
        ...
        success: function (response) {
                unblockUI();
                if (response && !Array.isArray(response) && response.Status === "N") {
                    toastr.error(response.Msg || "An error occurred while importing the data.");
                    return;
                }
        ...
        success: function (response) {
            if (response) {
                if (response.MRNDetails && response.MRNDetails.length > 0) {
                    $("#MRNTable").show();
                    const StringFilterColumn = [G_ItemConfig[0].ItemCodeHeader ? G_ItemConfig[0].ItemCodeHeader : 'Item Code', G_ItemConfig[0].ItemNameHeader ? G_ItemConfig[0].ItemNameHeader : 'Item Name'];
                    ...
                    if (G_IsUnloading == 'N') {
                        hiddenColumns.push("CaseNo");
                    }
                    if (G_IsUnloading == 'Y') {
                        StringFilterColumn.push("CaseNo");
                    }
                    const ColumnAlignment = {
                        BillQty:"right",
                        ReceivedQty:"right"
