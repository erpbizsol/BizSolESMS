DROP PROCEDURE IF EXISTS `USP_DispatchOrder`;

DELIMITER $$

CREATE PROCEDURE `USP_DispatchOrder`(
...
BEGIN 
    ...
    IF p_Mode = 'SAVE' THEN 
				...
				VALUES (
					...
					(SELECT Code FROM accountmaster WHERE AccountName = JSON_UNQUOTE(JSON_EXTRACT(p_jsonData, '$[0].clientName'))),
					JSON_UNQUOTE(JSON_EXTRACT(p_jsonData, '$[0].vehicleNo')), 
                    (SELECT Code FROM employeemaster WHERE EmployeeName = JSON_UNQUOTE(JSON_EXTRACT(p_jsonData, '$[0].packedBy'))),
					v_FinYear, 
					p_UserMaster_Code, 
					NOW(),  
					'Y' ,
                ...
                WHILE v_counter < v_total DO
					...
					Select DispatchQty Into v_DispatchQty From OrderDetailMaster
					WHERE ItemMaster_Code = (SELECT Code FROM itemmaster WHERE ItemName = JSON_UNQUOTE(JSON_EXTRACT(p_jsonData1, CONCAT('$[', v_counter, '].itemName'))))
					AND OrderMaster_Code = (SELECT Code FROM OrderMaster WHERE OrderNo = JSON_UNQUOTE(JSON_EXTRACT(p_jsonData1, CONCAT('$[', v_counter, '].orderNo'))));
    ...
    ELSEIF p_Mode = 'DELETE' THEN
        ...
        DELETE FROM DispatchDetailMaster WHERE DispatchMaster_Code = p_Code;
        DELETE FROM DispatchMaster WHERE Code = p_Code;
    ...
    ELSEIF p_Mode = 'SHOWDATA' THEN
        ...
        FROM DispatchMaster 
        LEFT JOIN AccountMaster ON DispatchMaster.AccountMaster_Code = AccountMaster.Code
       ...
       LEFT JOIN employeemaster ON DispatchMaster.EmployeeMaster_Code = employeemaster.Code
        ...
        LEFT JOIN UOMMaster ON itemmaster.UOMMaster_Code = UOMMaster.Code
        LEFT JOIN locationMaster ON itemmaster.locationMaster_Code = locationMaster.Code
        WHERE DispatchDetailMaster.DispatchMaster_Code =p_Code;
    ...
    ELSEIF p_Mode = 'LOCATE' THEN
			...
			FROM 
			DispatchMaster AS mm
			LEFT JOIN  AccountMaster AS am ON mm.AccountMaster_Code = am.Code
	...
	ELSEIF p_Mode = 'GETORDERNO' THEN
			Select OrderNo,OrderNoWithPrefix From OrderMaster 
   ...
   ELSEIF p_Mode = 'DespatchTransit' THEN
	...
	ELSEIF p_Mode = 'CompletedDespatch' THEN
		...
		Case When dispatchmaster.IsDispatched='Y' THEN 'Dispatched' ELSE 'Dispatch Pending' END AS Status,
        IFNULL(OrderMaster.Remark,'') As Remark
		FROM OrderMaster 
		LEFT JOIN dispatchmaster ON dispatchmaster.OrderMaster_Code = OrderMaster.Code
		LEFT JOIN dispatchdetailmaster ON dispatchdetailmaster.DispatchMaster_Code = dispatchmaster.Code
