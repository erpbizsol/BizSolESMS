...
CREATE PROCEDURE `USP_SaveScanDispatchOrder`(
...
proc_label: BEGIN
    ...
    SELECT COALESCE(IsPickingEnable, 'N') INTO v_IsPickingEnable FROM Fixparameter LIMIT 1;

    IF p_Mode IN ('SCAN', 'PICK') THEN
        IF UPPER(TRIM(IFNULL(p_Mode, ''))) = 'PICK' THEN
            SET p_BoxNo = 0;
        END IF;
